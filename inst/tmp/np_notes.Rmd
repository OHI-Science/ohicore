...
```{r init, echo=FALSE}
# setwd('~/Github_Mac/ohicore/inst/tmp'); library(knitr) # setwd('~/Github_Mac/ohicore')
...
yr_max = 2011 # 2014: yr_max=2011 # 2013: yr_max = 2010 # 2012: yr_max = 2011
...
Of all the region-product-year data (nrow=**`r nrow(h)`**), only the following # of rows were NA per variable (ie after initial begin of non-NA data and possible trailing extension):
...
h_na = h %.% 
  ...
  select(year, na)
kable(rename(as.data.frame(table(h_na)), c('na'='var', 'Freq'='NAs')))
```
