...
Of all the region-product-year data (nrow=**`r nrow(h)`**), only the following # of rows were NA per variable when merged with each other:
```{r, echo=FALSE, dependson='init', results='asis'}
# show where NAs usd vs tonnes
h_na = h %.% 
  filter(is.na(usd) | is.na(tonnes)) %.% 
  mutate(var = ifelse(is.na(usd) & is.na(tonnes), 'both', 
