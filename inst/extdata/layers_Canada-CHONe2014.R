...
ico_spp_status = read.csv('layers.Global2013.www2013/ico_spp_extinction_status.csv', stringsAsFactors=F); head(ico_spp_extinction_status)
...
# write out new layer
write.csv(ico_spp_status, 'layers.Global2013.www2013/ico_spp_extinction_status_value_Canada-CHONe.csv', na='', row.names=F)

# read layers.csv
layers = read.csv('layers.Global2013.www2013.csv', na.strings='', stringsAsFactors=F); head(layers); 

# alter fields for this updated layer
i = which(layers$layer=='rnk_ico_spp_extinction_status')
layers$layer[i]     = 'rnk_ico_spp_extinction_status_value'
layers$name[i]      = 'IUCN extinction risk value for iconic species'
