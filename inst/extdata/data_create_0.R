...
for (s in 2:3){ # s=2
  ...
  # copy files
  dir.to     = sprintf('inst/extdata/layers.Global201%d.vWWW2013', s, s)
  dir.create(dir.to, showWarnings=F)
  cat(sprintf('  copy to %s\n', dir.to))
  g$filename = g[, sprintf('fn_201%da' ,s)]
  g$path = file.path(dir.from.root, g[, sprintf('dir_201%da',s)], g$filename)
  ...
  g$targets = gsub('pressures', 'Pressures',
                   gsub('resilience', 'Resilience',
                        gsub('_', ' | ', as.character(g$target), fixed=T)))
  g$description = g$citation.2012n
  g$citation = g$X
  
  write.csv(g[,c('targets','layer','title','description','citation','units','filename','directory')], layers.csv, row.names=F, na='')
