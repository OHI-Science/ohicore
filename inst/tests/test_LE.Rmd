...
## Finished status with georegional gapfill (TODO: trend)
...
if (exists('georgndebug')){ rm(georgndebug)}
for (g in c('LIV','ECO','LE')){ # g='LIV'
...
# compare original scores (o) to most recent scores
rgn_labels = SelectLayersData(layers, layers='rtk_rgn_labels') %.%
  select(region_id=id_num, region_name=val_chr)

csv_o = '/Users/bbest/Desktop/neptune_cache/git-annex/Global/NCEAS-OHI-Scores-Archive/scores/scores.Global2013.www2013_2013-10-09.csv'
o = read.csv(csv_o, na.strings='NA', row.names=1) %.% 
  filter(goal %in% c('LIV','ECO','LE') & dimension %in% c('status','trend') & region_id!=0) %.% 
...
vs = scores %.%
  ...
  merge(georgndebug, by=c('goal','dimension','region_id')) %.%
  mutate(
    score_dif    = score - score_o,
    score_notna  = is.na(score)!=is.na(score_o))

vs_dif =  vs %.%
  filter(abs(score_dif) > 0.01) %.%
  arrange(goal, dimension, desc(abs(score_dif))) %.%
  select(goal, dimension, region_id, region_name, score, score_o, score_dif)
cat(sprintf('\nOf %d rows, %d rows had differing scores, ordered by goal, dimension, desc(abs(score_dif))\n', nrow(vs), nrow(vs_dif)))
print(vs_dif, row.names=F)
write.csv(vs_dif, sprintf('inst/tests/tmp_LE_o2013-10-09_vs_%s_dif.csv', format(Sys.Date(), '%Y-%m-%d')), na='', row.names=F)

vs_na =  vs %.%
  filter(score_notna == T) %.%
  arrange(goal, dimension, desc(abs(score)), desc(abs(score_o))) %.%
  select(goal, dimension, region_id, region_name, score, score_o)  
cat(sprintf('\nOf %d rows, %d rows had differing scores, ordered by goal, dimension, desc(abs(score)), desc(abs(score_o))\n', 
            nrow(vs), nrow(vs_na)))
print(vs_na, row.names=F)
write.csv(vs_na, sprintf('inst/tests/tmp_LE_o2013-10-09_vs_%s_na.csv', format(Sys.Date(), '%Y-%m-%d')), na='', row.names=F)
